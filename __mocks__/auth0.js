  ...
  constructor () {
    ...
    this.users = {
      updateAppMetadata: jest.fn(
        (identifierObj, updateObj) =>
          new Promise((resolve, reject) => {
            const match = this.__mockDB.user.find((user) => user.id === identifierObj.id)
            if (!match) {
              return reject(
                new Error(`User ${identifierObj.id} not found in DB (did you create them yet?`)
              )
            }
            if (Object.keys(identifierObj).length !== 1) {
              return reject(
                new Error(
                  `identifierObj expects only the id field got ${JSON.stringify(identifierObj)}`
                )
              )
            }
            ...
            if (!Array.isArray(updateObj.tms)) {
              return reject(
                new Error(`That is not a valid tms to give to auth0, ${JSON.stringify(updateObj)}`)
              )
            }
            resolve()
          })
