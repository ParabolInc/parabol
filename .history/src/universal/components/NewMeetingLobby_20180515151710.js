...
import ShowIfInitiallyTrue from 'universal/components/ShowIfInitiallyTrue';
...
const NewMeetingLobby = (props: Props) => {
  ...
  const {retroMeetingsOffered = 3, retroMeetingsRemaining = 0} = organization;
  ...
  return (
    <Lobby>
      ...
      <ShowIfInitiallyTrue criteria={!isPro}>
          {(updateInitialValue) => (
            <StyledInlineAlert>
      ...
      <ButtonGroup>
        <ButtonBlock>
          {(isPro || retroMeetingsRemaining > 0) &&
              ...
              <ShowIfInitiallyTrue criteria={!isPro && retroMeetingsRemaining === 0}>
                {(updateInitialValue) => (
            <LoadableModal
                LoadableComponent={UpgradeModalRootLoadable}
                maxWidth={350}
