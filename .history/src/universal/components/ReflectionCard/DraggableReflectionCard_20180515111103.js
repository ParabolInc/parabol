...
class DraggableReflectionCard extends React.Component<Props> {
  ...
  render () {
    ...
    console.log('drag ctx', isDragging, dragContext)
    const isTeamMemberDragging = !isDragging && Boolean(dragContext && dragContext.dragCoords && dragContext.draggerUserId !== viewerId)
    if (isTeamMemberDragging) {
      console.log('isTeam', dragContext)
    }
    return (
      <React.Fragment>
        {connectDragSource(
