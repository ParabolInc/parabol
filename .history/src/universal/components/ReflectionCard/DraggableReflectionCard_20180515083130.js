...
class DraggableReflectionCard extends React.Component<Props> {
  ...
  render () {
    ...
    const {draggerUserId, dragCoords} = reflection
    const isTeamMemberDragging = draggerUserId && draggerUserId !== viewerId && Boolean(dragCoords)
    console.log('isDragg', isDragging, Boolean(dragCoords))
    return (
      <React.Fragment>
        {connectDragSource(
          <div>
        ...
        <Modal isOpen={isDragging || Boolean(dragCoords)}>
