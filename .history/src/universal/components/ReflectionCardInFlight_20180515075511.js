...
class ReflectionCardInFlight extends React.Component<Props> {
  ...
  setDragState = (e) => {
    ...
    if (x !== dragCoords.x || y !== dragCoords.y) {
      commitLocalUpdate(atmosphere, (store) => {
        ...
        const dragCoords = createProxyRecord(store, 'Coords2D', {x, y})
        reflection.setLinkedRecord(dragCoords, 'dragCoords')
        console.log('set drag coords', {x, y})
      })
      const input = {
  ...
  render () {
    ...
    console.log('dragCoords', dragCoords)
