...
class ReflectionCardInFlight extends React.Component<Props> {
  ...
  setViewerDragCoords = (dragX, dragY) => {
    this.setState({
      dragX,
      dragY
    })
  ...
  setDragState = (e) => {
    ...
    if (!dragCoords || x !== dragCoords.x || y !== dragCoords.y) {
      commitLocalUpdate(atmosphere, (store) => {
        ...
        dragContext.setLinkedRecord(dragCoordsProxy, 'dragCoords')
      })
      const input = {
  ...
  render () {
    ...
    // console.log('dragCtx', dragContext)
    if (!dragContext || !dragContext.dragCoords) {
      return null
    }
    const {dragCoords, draggerUser} = dragContext
