...
import {Button, IconAvatar, Type} from 'universal/components';
...
import {cardTypeLookup, CCValidationErrors, stripeFieldLookup} from 'universal/utils/creditCardLookup';
...
class UpgradeCreditCardForm extends React.Component<Props, State> {
  ...
  secureSubmitCC = async () => {
    ...
    const {error, id: stripeToken} = await createToken({number: creditCardNumber, exp_month: expMonth, exp_year: expYear, cvc});
    if (error) {
  ...
  render() {
    ...
    return (
      <ModalBody>
        ...
        <Type align="center" colorPalette="dark" lineHeight="1.875rem" marginBottom=".25rem" scale="s6">
          {actionLabel}
        </Type>
        <Type align="center" colorPalette="dark" lineHeight={appTheme.typography.s5} scale="s3">
          <LockIcon name="lock" />{' Secured by '}<b>{'Stripe'}</b>
        </Type>
...
export default withAtmosphere(
  withAsync({'https://js.stripe.com/v2/': stripeCb})(
    withMutationProps(UpgradeCreditCardForm)
  )
);
