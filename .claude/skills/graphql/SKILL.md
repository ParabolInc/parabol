...
### TypeScript checking
Use `npx tsc -p packages/server/tsconfig.json --noEmit`. As of TypeScript 7, `tsc` *is* the native
(Go) compiler, so `tsgo` / `@typescript/native-preview` are gone — don't reintroduce them.
