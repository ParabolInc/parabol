        ...
        {isActive ? (
          ...
          <ProviderActions>
            <StyledButton
              ...
              waiting={submitting}
            >
              {!isConnectClicked && (isDesktop ? 'Setup' : <Icon>add</Icon>)}
              {isConnectClicked && (isDesktop ? 'Cancel' : <Icon>close</Icon>)}
            </StyledButton>
          </ProviderActions>
        )}
      </CardTop>
      {(isActive || isConnectClicked) && <JiraServerPanel teamId={teamId} viewerRef={viewer} />}
    </ExtraProviderCard>
  )
}
