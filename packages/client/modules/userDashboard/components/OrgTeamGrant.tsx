  ...
  return (
    <div className='flex flex-col gap-2'>
      <div className='flex items-center gap-2'>
        <span className='font-medium text-fg-primary text-sm'>Organizations & Teams</span>
        <RadioGroup
          value={orgGrantMode}
      ...
      {orgGrantMode === 'custom' && (
        <div className='rounded-md border border-hairline'>
          {organizations.length === 0 ? (
            <p className='p-3 text-fg-muted text-sm'>No organizations found</p>
          ) : (
            organizations.map((org) => {
              ...
              return (
                <div key={org.id} className='border-hairline border-b last:border-0'>
                  <div className='flex items-center gap-2 px-3 py-2'>
                    <label className='flex flex-1 cursor-pointer select-none items-center gap-2'>
                      ...
                      />
                      <span className='font-medium text-fg-primary text-sm'>{org.name}</span>
                    </label>
                    {org.teams.length > 0 && (
                      <button
                  ...
                  {isExpanded && org.teams.length > 0 && (
                    <div className='border-hairline border-t bg-surface-raised'>
                      {org.teams.map((team) => (
                        <label
                          key={team.id}
                        ...
                        >
                          ...
                          />
                          <span className='text-fg-secondary text-sm'>{team.name}</span>
                        </label>
                      ))}
                    </div>
                  )}
