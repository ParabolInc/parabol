...
const TeamPromptDraftingCard = (props: Props) => {
  ...
  return (
    <div className='mx-auto flex w-full max-w-[600px] flex-col'>
      ...
      <div className='flex min-h-[92px] flex-col justify-center gap-1 rounded-card bg-surface-well p-4 text-fg-muted'>
        <div className='flex items-center gap-2 font-semibold text-sm'>
          {!isEnded && <Create className='h-4 w-4' />}
          {title}
        </div>
        {!isEnded && (
          <div className='text-xs'>You'll see {preferredName}'s update when they share it</div>
        )}
      </div>
    </div>
  )
