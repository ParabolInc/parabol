...
import {DatabaseBlockBase} from '../../../shared/tiptap/extensions/DatabaseBlockBase'
import {DatabaseBlockView} from './DatabaseBlockView'

declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    moveDatabase: {
      moveDatabase: (params: {fromIndex: number; toIndex: number}) => ReturnType
    }
  }
  interface Storage {
    databaseBlock: DatabaseBlockStorage
  }
}

interface DatabaseBlockStorage {
  yDoc: Y.Doc
}
export const DatabaseBlock = DatabaseBlockBase.extend<{yDoc: Y.Doc}, DatabaseBlockStorage>({
  addOptions: () => ({
      ...
      moveDatabase:
        ...
        ({state, dispatch}) => {
          ...
          if (fromIndex < 0 || fromIndex >= blockCount) {
            console.error(`moveDatabase: Invalid fromIndex ${fromIndex}`)
            return false
          }
      ...
      new Plugin({
        props: {
          ...
          transformCopied(slice: Slice): Slice {
            function decanonDatabases(frag: Fragment): Fragment {
            ...
            // return slice
            const newContent = decanonDatabases(slice.content)
