...
export const SelectTrigger = React.forwardRef<HTMLButtonElement, SelectTriggerProps>(
  ({className, children, isLoading, iconClassName, asChild, ...props}, ref) => {
    // the caller owns the whole control, chevron included
    if (asChild) {
      return (
        <RadixSelect.Trigger asChild ref={ref} className={className} {...props}>
          {children}
        </RadixSelect.Trigger>
      )
    }
