function addVote(column, cardIndex, voteIndex) {
  ...
  cy.get(`[data-cy=group-column-${column}-body]`).children().eq(cardIndex).find(`[data-cy=reflection-vote-row]`)
    .as('vote-card')
  
    cy.get('@vote-card').find(`[data-cy=add-vote]`)
      .as('add-vote')

    cy.get('@add-vote').click()

    cy.get('@vote-card').find(`[data-cy=completed-vote-${voteIndex}]`).should('exist')
...
function removeVote(column, cardIndex, voteIndex) {
  ...
  cy.get(`[data-cy=group-column-${column}-body]`).children().eq(cardIndex).find(`[data-cy=reflection-vote-row]`)
    ...
    cy.get('@vote-card').find(`[data-cy=completed-vote-${voteIndex}]`).should('exist').click()

    cy.get('@vote-card').find(`[data-cy=add-vote]`).should('exist')
...
describe('Test Vote page Demo', () => {
  ...
  before(function() {
    ...
    cy.visitReflect().visitPhase('group').visitPhase('vote')

  })

  it("Check that all personal votes are remaining and no team votes have been used", () => {
    cy.get(`[data-cy=my-votes-remaining]`).children().should('have.length', 5)
