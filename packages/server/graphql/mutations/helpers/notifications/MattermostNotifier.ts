...
const notifyMattermost = async (
  ...
  console.log('notifyMattermost', event, channel, user, teamId, textOrAttachmentsArray, notificationText)
...
type MattermostNotificationAuth = IntegrationProviderMattermost & {userId: string, teamId: string, channel: string | null}
...
const MattermostNotificationHelper: NotificationIntegrationHelper<MattermostNotificationAuth> = (
  ...
  async endTimeLimit(meeting, team, user) {
    ...
    return notifyMattermost('MEETING_STAGE_TIME_LIMIT_END', notificationChannel, user, team.id, attachments)
  },
...
async function getMattermost(dataLoader: DataLoaderWorker, teamId: string, userId: string) {
  ...
  return Promise.all(filteredAuths.map(async (auth) => {
    const provider = (await dataLoader.get('integrationProviders').loadNonNull(auth.providerId)) as IntegrationProviderMattermost
