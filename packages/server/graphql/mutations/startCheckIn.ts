...
export default {
  ...
  deprecated: 'Use specfic start[meetingType] instead',
  args: {
    teamId: {
  ...
  async resolve(
    ...
    const otherActiveMeeting = newActiveMeetings.find((activeMeeting) => {
      ...
      if (id === meetingId || activeMeeting.meetingType !== MeetingTypeEnum.action) return false
    ...
    await Promise.all([
      ...
      r
        .table('Team')
        .get(teamId)
        .update({lastMeetingType: MeetingTypeEnum.action})
        .run(),
      r
        .table('AgendaItem')
