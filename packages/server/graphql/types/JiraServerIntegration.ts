...
const JiraServerIntegration = new GraphQLObjectType<any, GQLContext>({
  ...
  fields: () => ({
    ...
    sharedProviders: {
      ...
      resolve: async ({teamId, userId}: {teamId: string, userId: string}, _args: unknown, {dataLoader}: GQLContext) => {
