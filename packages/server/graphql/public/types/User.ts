...
const User: ReqResolvers<'User'> = {
  ...
  invoices,
  archivedTasks: async (_source, {first, after, teamId}, {authToken, dataLoader}) => {
    // AUTH
    const userId = getUserId(authToken)
    if (!(await isTeamMemberAsync(userId, teamId, dataLoader))) {
  ...
  archivedTasksCount: async (_source, {teamId}, {authToken, dataLoader}) => {
    const pg = getKysely()
    const viewerId = getUserId(authToken)
  ...
  teams: async ({id: userId}, {includeArchived}, {authToken, dataLoader}) => {
    ...
    if (viewerId === userId || isSuperUser(authToken)) {
      ...
      const viewerTeamMembers = await dataLoader.get('teamMembersByUserId').load(viewerId)
      const viewerTeamIds = new Set(viewerTeamMembers.map(({teamId}) => teamId))
      activeTeamIds = userActiveTeamIds.filter((teamId) => viewerTeamIds.has(teamId))
    }
    const teamIds = includeArchived ? userActiveTeamIds : activeTeamIds
    const teams = (
      await Promise.all(
  ...
  teamMember: async ({id}, {teamId, userId}, {authToken, dataLoader}) => {
    const viewerId = getUserId(authToken)
  ...
  userOnTeam: async (_source, {userId}, {authToken, dataLoader}) => {
    ...
    if (!tms.find((teamId) => viewerTeamIds.has(teamId))) return null
    return userOnTeam
  },
  ...
  canAccess: async (_source, {entity, id}, {authToken, dataLoader}) => {
    ...
    switch (entity) {
      case 'Team':
        return isTeamMemberAsync(viewerId, id, dataLoader)
      case 'Meeting': {
