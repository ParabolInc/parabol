...
const fetchAzureDevopsProjects = async (dataLoader: DataLoaderWorker, teamId, userId) => {
  ...
  const azureDevopsProjects = [] as Omit<ISuggestedIntegrationAzureDevops, '__typename' | 'remoteProject'>[]
  ...
  await manager.getProjects(organizations, (err, res) => {
    if (err) {
      ...
      const newItems = newProjects.map((project) => ({
        ...
        id: makeSuggestedIntegrationId({...project, projectKey: project.key, organization, service}),
        service,
        organizations,
