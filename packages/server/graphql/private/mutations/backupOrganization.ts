...
const backupOrganization: MutationResolvers['backupOrganization'] = async (
  ...
  await r({
    ...
    meetingSettings: (r.table('MeetingSettings').getAll(r.args(teamIds), {index: 'teamId'}) as any)
      .coerceTo('array')
    ...
    meetingTemplate: (r.table('MeetingTemplate').getAll(r.args(teamIds), {index: 'teamId'}) as any)
      .coerceTo('array')
    ...
    userIds: r
      ...
      .do((userIds: RValue) => {
        return r({
          notification: (r.table('Notification').getAll(r.args(userIds), {index: 'userId'}) as any)
