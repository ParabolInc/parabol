...
export default {
  ...
  resolve: async (_source, _args, {authToken, socketId}: GQLContext) => {
    ...
    console.log('disconnect -- userPresence', userPresence)
    ...
    if (userPresence.length === 1) {
      ...
      console.log('disconnect listeningUserIdsArr', listeningUserIdsArr)
      listeningUserIdsArr.forEach((onlineUserId) => {
        console.log('disconnect onlineUserId', onlineUserId)
        publish(
