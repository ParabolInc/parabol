...
import appOrign from '../appOrigin'
interface Props {
  meetingId: string
  context: GQLContext
}
...
const newMeetingSummaryEmailCreator = async (props: Props) => {
  ...
  const MeetingSummaryEmailRootSSR = require('./components/MeetingSummaryEmailRootSSR').default
  const bodyContent = await renderSSRElement(
    <MeetingSummaryEmailRootSSR appOrign={appOrign} environment={environment} meetingId={meetingId} />,
    environment
  )
