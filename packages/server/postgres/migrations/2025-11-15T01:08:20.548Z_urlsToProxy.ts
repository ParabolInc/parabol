...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await Promise.all([
    db
      ...
      .set({
        ...
        rasterPicture: sql`REPLACE("rasterPicture", ${prefix}, '/assets')`
      })
      .where((eb) =>
  ...
  // we still need to go through every Page & Reflection, extract the pictures, move them to the right AssetScope, change the scopeId, re-upload & update the doc
}
