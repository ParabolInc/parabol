...
const getBestTeamIntegrationAuthQueryIR: any = {
  name: 'getBestTeamIntegrationAuthQuery',
  params: [
    {
      name: 'userId',
      transform: {type: 'scalar'},
      codeRefs: {used: [{a: 68, b: 73, line: 4, col: 22}]}
    },
    {
      name: 'teamId',
      transform: {type: 'scalar'},
      codeRefs: {used: [{a: 138, b: 143, line: 5, col: 18}]}
    },
    {
      name: 'service',
      transform: {type: 'scalar'},
      codeRefs: {used: [{a: 162, b: 168, line: 6, col: 17}]}
    }
  ],
  usedParamSet: {userId: true, teamId: true, service: true},
  statement: {
    body: 'SELECT *, "userId" = :userId as "isUser" FROM "TeamMemberIntegrationAuth"\nWHERE "teamId" = :teamId\nAND "service" = :service\nAND "isActive" = TRUE\nORDER BY "isUser" DESC, "updatedAt" DESC\nLIMIT 1',
    loc: {a: 46, b: 239, line: 4, col: 0}
  }
}
 ...
 * ```
 */
export const getBestTeamIntegrationAuthQuery = new PreparedQuery<
  IGetBestTeamIntegrationAuthQueryParams,
  IGetBestTeamIntegrationAuthQueryResult
>(getBestTeamIntegrationAuthQueryIR)
