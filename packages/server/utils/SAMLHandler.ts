...
const query = `
mutation LoginSAML($queryString: String!, $clientSlug: String!) {
  loginSAML(queryString: $queryString, clientSlug: $clientSlug) {
    error {
      message
    }
    authToken
...
const SAMLHandler = uWSAsyncHandler(async (res: HttpResponse, req: HttpRequest) => {
  const clientSlug = req.getParameter(0)
  if (!clientSlug) {
    redirectOnError(res, 'No client slug Provided!|Did you set up the service provider correctly?')
    return
  }
  const parser = (buffer: Buffer) => buffer.toString()
  const queryString = await parseBody({res, parser})
  const payload = await publishWebhookGQL(query, {clientSlug, queryString})
  if (!payload) return
