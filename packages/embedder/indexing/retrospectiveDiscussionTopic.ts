...
import {meetingSeries} from 'parabol-server/dataloader/primaryKeyLoaderMakers'
  ...
  do {
    ...
    const {maxCreatedAt, completedNewMeetings} = await r
      ...
      .do((rows) => ({
        ...
        completedNewMeetings: r.expr(rows).filter((r) =>
          r('meetingType')
            .eq('retrospective')
            .and(
              r('endedAt').gt(0),
    ...
    const embeddingsMetaRows = (
      await Promise.all(
        completedNewMeetings.map((m) => newRetroDiscussionTopicsFromNewMeeting(m, dataLoader))
      )
    ).flat()
