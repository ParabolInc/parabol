...
## Iconic [relay] tier approach (Task #4, do after presentational tier)
Signature Relay components (PokerCard, ReflectionCard, TeamPromptResponseCard, DashboardAvatars, OrgPlan) read data via `useFragment(graphql`…`, ref)`. Plan: alias react-relay's READ hooks to identity in a stub (`useFragment = (_f, ref) => ref`, and passthrough for usePreloadedQuery/useLazyLoadQuery/usePaginationFragment/useRefetchableFragment) via `.design-sync/tsconfig.ds.json` paths, so the component renders directly from a plain data object passed as the "ref" in the preview (e.g. `<PokerCard scaleValue={{color:'#e91e63', label:'5'}}/>`). The stub must import the REAL react-relay for non-read exports without self-aliasing (import via `react-relay/lib/…` or a node_modules-absolute path). Add these components to the manifest as a group with `includeRelay:true`. Some (drag-driven, editor-driven) may still only floor-card.
