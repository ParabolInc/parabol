...
import {NewMeetingPhase} from 'universal/types/graphql'
import findStageById from 'universal/utils/meetings/findStageById'

const getNextFacilitatorStage = (
  facilitatorStageId: string,
  teamMemberStageId: string,
  phases: ReadonlyArray<NewMeetingPhase>
) => {
...
const removeTeamMemberFromNewMeeting = async (teamMemberId, teamId, dataLoader) => {
  ...
  if (meetingId) {
    ...
    if (nextStageId !== facilitatorStageId) {
      // mutative
