...
export default async function setIsEnterprise() {
  ...
  console.log('Updating tier to "enterprise" for Organization and OrganizationUser tables in RethinkDB')
  ...
  const updateUserPromise = pg.query(`UPDATE "User" SET tier = 'enterprise' RETURNING id`).then(res => {
    console.log('Updated User tier to enterprise for:', res.rows.length, 'records in PostgreSQL.')
    return res
  })

  const updateTeamPromise = pg.query(`UPDATE "Team" SET tier = 'enterprise' RETURNING id`).then(res => {
